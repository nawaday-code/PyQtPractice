...
class DataSender(Members):
    ...
    DFyakinhyou, 夜勤表
             4           5,        6,        0,         1,          2,        3,         
        "Angio夜勤", "MRI夜勤", "CT夜勤","Angio日勤", "MRI日勤", "CT日勤", "Free日勤", 
    ...
    def getDf4Iwasaki(self):
        ...
        deptdf = pd.DataFrame({uid:person.dept for uid, person in self.members.items()}, index=[0])
        logging.debug(deptdf)
        deptdf.to_csv('dist/deptDF.csv')
